...
def select_dropdown_by_text(driver, toggle_index, target_text):
    ...
    for attempt in range(3):
        ...
        if items:
            ...
            # ✅ Verifikasi dropdown yang sama (FIX)
            val = driver.execute_script("""
                var el = arguments[0];
                var parent = el.closest('.jr-mSingleselect');
                if(!parent) return '';
                var sel = parent.querySelector('.jr-mSingleselect-input-selection');
                if(!sel) return '';
    ...
    print(f"  ⚠️  '{target_text}' tidak ditemukan, lanjut...")
    return True
